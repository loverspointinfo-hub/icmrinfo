...
async def search(
    ...
    # Add detected info if available
...
def search_ui(query: str, limit: int, mode: str):
    """Enhanced UI with response time display"""
    ...
    # Build status bar with timing
    cache_status = "✅" if from_cache else "❌"
    timing_status = "⚡" if total_time < 50 else "🐢" if total_time < 200 else "⏳"
...
📋 **Mode:** {detected_mode}
💾 **Cache:** {cache_status}
⏱️ **Response:** {total_time}ms (search: {search_time}ms)
...
📋 **Mode:** {detected_mode}
💾 **Cache:** {cache_status}
⏱️ **Response:** {total_time}ms (search: {search_time}ms) {timing_status}
...
---\n\n"""
    ...
    # Add footer with response time
    footer = f"\n\n---\n⏱️ **Total Response Time:** {total_time}ms | **Search Time:** {search_time}ms | **Cache:** {cache_status}"
...
def build_ui():
    with gr.Blocks(
        ...
        gr.Markdown("Search **2.5 billion records** with response time tracking", elem_classes="subtitle")
        
        with gr.Row():
            with gr.Column(scale=3):
                query_input = gr.Textbox(
                    label="Search Query",
                    placeholder="Phone, Aadhaar, name, or address...",
                    lines=1,
                )
...
**Source:** [HF Dataset](https://huggingface.co/datasets/Kzr0xx/icrm-hitek-full-db-mixed)
        ...
        gr.Markdown(
            ...
            "⚡ **v2.0.0** with response time tracking"
            "</div>",
            elem_classes="footer"
        )
