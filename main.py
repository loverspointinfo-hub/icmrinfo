...
# Set environment variables early for DuckDB
os.environ.setdefault("HOME", "/tmp")
os.environ.setdefault("DUCKDB_HOME", "/tmp")

# Reduce memory usage for Vercel
os.environ.setdefault("ICMR_PARALLEL", "1")  # Reduce parallel connections
os.environ.setdefault("ICMR_THREADS_PER_CONN", "1")  # Single thread per connection
...
PARALLELISM = int(os.environ.get("ICMR_PARALLEL", "1"))  # Reduced for Vercel
THREADS_PER_CONN = int(os.environ.get("ICMR_THREADS_PER_CONN", "1"))  # Reduced for Vercel
DUPLICATE_CAP = 2
CACHE_SIZE = 100  # Reduced for Vercel memory limits
CACHE_TTL = 300
...
# Remote indexes (only load when needed)
REMOTE_INDEXES = {
...
def _init_duckdb():
    ...
    with _conn_lock:
        ...
        try:
            ...
            con.execute("SET max_memory='256MB'")  # Limit memory usage
            ...
            # Create views - but don't load data yet
            for kind, urls in REMOTE_INDEXES.items():
                view = f"people_{kind}"
                try:
                    # Create view with lazy loading
            ...
            # Return a minimal connection for sample data
    ...
    try:
        ...
        for i in range(50):  # Reduced for Vercel
            ...
            try:
                ...
                """, [
                    name,
                    sample_fathers[i % len(sample_fathers)],
                    phone,
                    aadhar,
                    f"8{i:08d}" if i % 3 == 0 else "",
                    f"{i+1}, {sample_towns[i % len(sample_towns)]} Main Road",
                    sample_towns[i % len(sample_towns)],
                    f"{100000 + i}",
        ...
        print(f"Created {min(50, len(sample_names))} sample records")
    except Exception as e:
...
def _run_field_search(field: str, value: str, mode: str, limit: int) -> dict:
    ...
    if field not in SEARCH_FIELDS:
        return {"field": field, "value": value, "mode": mode, "count": 0, "results": [], "error": "Invalid field"}
    ...
    # Simple search without complex SQL (avoids Vercel issues)
    con = _get_conn()
    if con is None:
        return {"field": field, "value": value, "mode": mode, "count": 0, "results": [], "error": "Database unavailable"}
    ...
    try:
        ...
        if mode == "exact":
            ...
            return {"field": field, "value": value, "mode": mode, "count": 0, "results": [], "error": "Invalid mode"}
...
def build_ui():
    with gr.Blocks(title="ICMR Search", theme=gr.themes.Soft()) as demo:
        ...
        with gr.Row():
            limit_slider = gr.Slider(minimum=1, maximum=20, value=10, step=1, label="Results")
        ...
        gr.Markdown("---")
        gr.Markdown("👨‍💻 **Developer:** @kzr0x | 📢 **Channel:** @api_wallah")
